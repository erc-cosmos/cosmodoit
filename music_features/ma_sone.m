...
%% elias 13.6.2004 - last edited by DB 25.11.2020
...
if strcmp(p.bark_type,'table')
    ...
    for i=1:cb
        [~, tmp.f_idx_upper(i)] = min(abs(tmp.bark - tmp.b_idx_upper(i)));
        [~, tmp.f_idx_center(i)] = min(abs(tmp.bark - tmp.b_idx_center(i)));
    end
...
switch p.outerear
    ...
    otherwise
        error(['unknown outer ear model: p.outerear = ',p.outerear]);
end
...
%    semilogx(c.fft_freq(2:end),10*log10(tmp.W_Adb_mT(2:end)),'b'); hold on
    legend({'Terhardt'},'Location','northwest')
